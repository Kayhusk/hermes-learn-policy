...
Keep a multi-file learning proposal coherent. If an owner/index change and support file are both required, do not persist only one half; prefer one self-contained native mutation or no write. After a native mutation rejection, one rejection ends this review: do not retry unchanged or create a sibling to route around the guard.
